...
void StartDefaultTask(void *argument)
{
  ...
  /* USER CODE BEGIN 5 */
	...
	/* Infinite loop */
	for (;;) {
		osMessageQueueGet(queueSubscriptionStatusHandle, &status, NULL, 0);
		connection_status = mqtt_client_is_connected(client);
		if (client != NULL && connection_status == 0 && status == 1) {
			interface_mqtt_connect(client, &ci, &data);
		}
		if (status == 0) {
			mqtt_disconnect(client);
		}
		if (data != 5) {
			osMessageQueuePut(queueZeroOneHandle, &data, 0, 0);
			data = 5;
		}
